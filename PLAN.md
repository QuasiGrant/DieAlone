...
- Interaction: derive from Interactable (Assets/Scripts/Interaction), set the prompt string, override Use. PlayerInteractor on the Player raycasts 2 m from the camera and calls Use on the Interact action. HUD uses uGUI with legacy Text; TextMeshPro needs its Essential Resources imported first, which is an asset add. The center dot exists as HUD/Dot but is deactivated: owner prefers the prompt word alone.
